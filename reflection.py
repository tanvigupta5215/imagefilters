...
def make_reflected(filename):
    image = SimpleImage('images/mt-rainier.jpg')
    # TODO: your code here.
    width = image.width
    height = image.height
    reflection_image = SimpleImage.blank(width, height * 2)
    for y in range(height):
        for x in range(width):
            pixel = image.get_pixel(x, y)
            reflection_image.set_pixel(x, y, pixel)
            reflection_image.set_pixel(x, (height * 2) - (y+1), pixel)
